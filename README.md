...
修改为一件运行脚本
    ```shell
    wget --no-check-certificate -o one-key-ikev2.sh https://raw.githubusercontent.com/aimo-x/one-key-ikev2-vpn/master/one-key-ikev2.sh|bash && chmod +x one-key-ikev2.sh && bash one-key-ikev2.sh
    ```
